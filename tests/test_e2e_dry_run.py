...
from scripts.worktree_manager import create_worktree, remove_worktree, get_worktree_path

@pytest.fixture
def clean_worktree():
    task_id = "TASK-E2E-TEST"
    repo_root = Path.cwd()
    yield task_id, repo_root
    # Cleanup fixture
    remove_worktree(repo_root, task_id, delete_branch=True)

def test_e2e_dry_run_success_flow(clean_worktree):
    task_id, repo_root = clean_worktree
    ...
    # Configurar comando de prueba local para el worktree
    ...
    # Limpiar worktree antes de merge a dev
...
def test_e2e_dry_run_with_retry_flow(clean_worktree):
    task_id, repo_root = clean_worktree
    spec_path = repo_root / "specs" / "TASK-001.md"
