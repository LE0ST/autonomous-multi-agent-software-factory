...
- [AC-01] Implementar una función `validate_password(password: str) -> bool` que rechace contraseñas vacías, menores de 8 caracteres y aquellas que no contengan al menos una letra y un número.
...
- [TEST-01] Verificar que `validate_password` rechaza valores vacíos, contraseñas menores de 8 caracteres y contraseñas que no contienen simultáneamente letras y números. (Cubre: [AC-01], [SEC-01])
- [TEST-02] Verificar que `validate_password` acepta contraseñas válidas y que no produce salida por stdout/stderr ni persiste la contraseña recibida. (Cubre: [AC-02], [SEC-02])
